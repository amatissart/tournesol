...
class MlInputFromPublicDataset(MlInput):
    def __init__(self, csv_file):
        ...
        self.public_dataset.rename({"video_a": "entity_a", "video_b": "entity_b"}, axis=1, inplace=True)
        self.public_dataset["user_id"], self.user_indices = self.public_dataset["public_username"].factorize()
    ...
    def get_ratings_properties(self):
        user_entities_pairs = pd.Series(iter(
    ...
    def get_ratings_properties(self):
        values = ContributorRating.objects.filter(
            ...
            is_supertrusted=Case(
                When(
                    user__in=User.supertrusted_users(),
                    then=True
                ),
                default=False
            ),
